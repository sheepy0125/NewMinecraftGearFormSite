...
""" Base Tkinter window """
class BaseWindow:
	def __init__(self, win_size: tuple=(300, 300), win_resizable: bool=False, win_title: str="Unnamed window", widget_size: Union[tuple, None]=None) -> None:
	...
	def label(self, text: str) -> Label:
		return Label(master=self.root, text=text, width=self.widget_size[0], height=self.widget_size[1])

	def button(self, text: str, on_click: Callable) -> Button:
		return Button(master=self.root, text=text, command=on_click, width=self.widget_size[0], height=self.widget_size[1])

	def text(self, state: Union[str, None]=None, rows: int=2):
		return Text(master=self.root, state=state, width=self.widget_size[0], height=rows)
